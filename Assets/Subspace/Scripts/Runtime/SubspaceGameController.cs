...
namespace Subspace
{
    public sealed class SubspaceGameController : MonoBehaviour
    {
        private const string ResourcesConfigPath = "Subspace/SubspaceGameConfig";
        ...
        private readonly List<SubspaceSymbolDefinition> runtimeSymbolPool = new List<SubspaceSymbolDefinition>();
        private readonly List<Image> cellImages = new List<Image>();
        private readonly List<Text> cellTexts = new List<Text>();
        private readonly List<SubspaceSymbolCellView> cellViews = new List<SubspaceSymbolCellView>();
        ...
        private static void AutoCreateController()
        {
            ...
            // by SubspaceComponentSceneBuilder and should not auto-create this monolithic controller.
            return;

#pragma warning disable CS0162
            if (FindObjectOfType<SubspaceGameController>() != null)
            {
                return;
            }

            var controllerObject = new GameObject("Subspace Runtime Bootstrap");
            controllerObject.AddComponent<SubspaceGameController>();
#pragma warning restore CS0162
        }

        public void SetConfig(SubspaceGameConfig newConfig)
        {
            config = newConfig;
        }

        public void SetArtRig(SubspaceArtRig newArtRig)
        {
            artRig = newArtRig;
        }
        ...
        private void LoadConfig()
        {
            if (artRig == null)
            {
                artRig = FindObjectOfType<SubspaceArtRig>(true);
            }
            ...
            {
                config = Resources.Load<SubspaceGameConfig>(ResourcesConfigPath);
            }
            ...
            for (int y = 0; y < rows; y++)
            {
                for (int x = 0; x < columns; x++)
                {
                    ...
                    if (tiles != null)
                    {
                        if (tiles[x, y] == null)
                        {
                            tiles[x, y] = new SubspaceTileData(x, y, board[x, y]);
                        }
                        else
                        {
                            tiles[x, y].currentSymbol = board[x, y];
                        }
                    }
                }
            }
        ...
        private IEnumerator AttackRoutine()
        {
            ...
            var scoreResult = tiles != null ? SubspaceScoreResolver.Calculate(tiles, selection) : SubspaceScoreResolver.Calculate(board, selection);
        ...
        {
            var existing = GameObject.Find("SubspaceCanvas");
            if (existing != null)
            {
                Destroy(existing);
            }

            var canvasObject = new GameObject("SubspaceCanvas", typeof(Canvas), typeof(CanvasScaler), typeof(GraphicRaycaster));
        ...
        private void BuildBoardCells()
        {
            ...
            for (int y = 0; y < rows; y++)
            {
                for (int x = 0; x < columns; x++)
                {
                    ...
                    var view = cellImage.gameObject.AddComponent<SubspaceSymbolCellView>();
                    view.Configure(cellImage, text);
                    cellViews.Add(view);
        ...
        private void RefreshBoardVisuals()
        {
            ...
            for (int y = 0; y < rows; y++)
            {
                for (int x = 0; x < columns; x++)
                {
                    ...
                    if (index >= cellViews.Count || symbol == null)
                    {
                        continue;
                    }

                    Sprite symbolSprite = GetSymbolSprite(symbol);
                    cellViews[index].SetTile(tiles != null ? tiles[x, y] : new SubspaceTileData(x, y, symbol), symbolSprite != null ? symbolSprite : SolidSprite);
                }
            }
        ...
        private SubspaceLevelDefinition GetLevel(int index)
        {
        ...
        private Sprite GetSymbolSprite(SubspaceSymbolDefinition symbol)
        {
        ...
        private Sprite SolidSprite
        {
            get
            {
                ...
                {
                    name = "SubspaceSolidPixel",
                    hideFlags = HideFlags.HideAndDontSave
                };
                texture.SetPixel(0, 0, Color.white);
                texture.Apply();
                solidSprite = Sprite.Create(texture, new Rect(0f, 0f, 1f, 1f), new Vector2(0.5f, 0.5f));
                solidSprite.name = "SubspaceSolidSprite";
                solidSprite.hideFlags = HideFlags.HideAndDontSave;
                return solidSprite;
            }
        }

        private static SubspaceGameConfig CreateFallbackConfig()
        {
            var config = ScriptableObject.CreateInstance<SubspaceGameConfig>();
            config.name = "Runtime Fallback SubspaceGameConfig";
        ...
        private static SubspaceArtSet CreateFallbackArtSet()
        {
            var art = ScriptableObject.CreateInstance<SubspaceArtSet>();
            art.name = "Runtime Fallback SubspaceArtSet";
            return art;
        }

        private static SubspaceSymbolDefinition CreateRuntimeSymbol(string id, string displayName, int score, Color color)
        {
            var symbol = ScriptableObject.CreateInstance<SubspaceSymbolDefinition>();
        ...
        private static SubspaceLevelDefinition CreateRuntimeLevel(string id, string displayName, int targetScore, int turns, SubspaceSymbolDefinition reward)
        {
            var level = ScriptableObject.CreateInstance<SubspaceLevelDefinition>();
            level.name = displayName;
            level.levelId = id;
            level.displayName = displayName;
            level.enemyTargetScore = targetScore;
            level.maxTurns = turns;
            level.boardColumns = 6;
            level.boardRows = 6;
